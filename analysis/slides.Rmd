...
```{r read-trip-data}
trips <- read.csv("../data/Divvy_Trips_2017_Q4.csv",
                  stringsAsFactors = FALSE)
```
...
just in case you accidentally quit or the program crashes; e.g.,
...
+ CRAN is the official package source (https://cran.r-project.org).

+ Other important sources: Bioconductor, GitHub.

+ What packages are already installed? Run `installed.packages()`.

+ Where are my packages? Run `.libPaths()`.

Inspect the Divvy trip data
===========================
