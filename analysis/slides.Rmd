...
+ **analysis**: directory where the code and data for our analyses
  will be stored.

+ **slides.pdf**: the workshop slides.

Open the slides on your computer
================================

+ This PDF is useful for copying & pasting code from the slides.
  (Note that copying multiple lines from PDF may not always work.)
...
x <- stations$name
trips$from_station_name <-
  factor(trips$from_station_name,x)
trips$to_station_name <-
  factor(trips$to_station_name,x)
```
...
What function `read.divvy.data` does:
