...
+ **Note:** Confusingly, `read_csv` automatically converts empty
...
*Why is the second approach better in this case?*
...
plot(tripcounts$from,tripcounts$to,pch = 20,
  col = "blue",panel.first = abline(a=0,b=1))
```
...
powerful (although not initially intuitive) set of plotting functions
that extend the base plotting fuctions in R.
...
Load the ggplot2 and cowplot functions into your environment:
...
p <- p + geom_point(shape = 21,fill = "limegreen",
                    color = "white")
p <- p + theme_cowplot()
print(p)
```
...
stations$dep.2016 <- c(x)
stations$dep.2017 <- c(y)
head(stations)
```
...
ggplot2 is relatively straightforward:
...
+ Therefore, we can write a script to *automate* our data analysis.
