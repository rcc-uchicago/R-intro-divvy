...
I wrote a function to read and prepare the 2016 & 2017 Divvy
data.
...
*How many more trips were taken in 2017 than in 2017? Use the
`summary` function to answer this.*
...
p <- ggplot(data = divvy$stations,
       mapping = aes(x = longitude,y = latitude)) +
  geom_point(shape = 21,fill = "darkblue",color = "white",size = 2)
print(p)
```

There are many, many functions in the ggplot2 package for adjusting
plotting settings. The website (ggplot2.tidyverbse.org) is a great
reference.
