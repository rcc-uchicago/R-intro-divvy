...
> Instructor notes: The "which" line of code is a pretty good example
> of a more complicated (compound) expression.
...
data from the 3rd and 4th quarters of 2017. How would we do that?
...
We can *merge* the two tables *by rows*---that is, we put one table on
top of the other. 
...
When merging data sets by rows, we have beware of inconsistencies in
the columns, e.g.,

```{r check-merge-gender}
class(tripsQ3$gender)
class(tripsQ4$gender)
class(trips$gender)
```

> Instructor notes: What other inconsistencies might we want to check?
>
> There are many, many ways different data sets can have
> inconsistencies that need to be fixed prior to merging.
>
> What mistake did we make in the "rbind" call above? (Hint: the
> reverse chronological ordering of the trips may not have been
> preserved.)
...
out <- geom_point(shape = 21,fill = "limegreen",
                  color = "white",size = 3)
p3  <- ggplot_add(out,p)
print(p3)
```
...
out <- scale_fill_gradient2(low = "dodgerblue",
...
p <- ggplot(stations,a)
out  <- geom_point(shape = 21,fill = "white",
                   color = "black")
p    <- ggplot_add(out,p)
print(p)
```
