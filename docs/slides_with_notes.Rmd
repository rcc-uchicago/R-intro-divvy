...
The "plot" function allows you to very simply create a scatterplot
from two numeric vectors:

```{r plot-stations-1}
plot(stations$longitude,stations$latitude,pch = 20)
```

The plot function has many, many options, which we will not explore here.
Let's add color to the plot according to the "city" column:
...
Now let's recreate the stations map, this time using **ggplot2**. It
is a powerful (though not immediately intuitive) plotting
interface. First, install ggplot2 (and cowplot, and extension to
ggplot2) if you have not already done so:
...
As you can see, the code is a bit more complicated:
...
What is improved in the new plot? 
...
I fiddled around with the `geom_point` settings to improve the plot a
bit:
...
here with the "scale" function `scale_fill_manual`. Also, I'm partial
to the theme from the cowplot package.
...
Observe that adjustments to the plot are made by adding "layers". This
is one of the distinctive features of ggplot2.

35. Save your code & session state
==================================
...
36. Main concepts covered so far
================================
